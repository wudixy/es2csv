...
def getAgg(jsobj,glist,vlist,data,getcount=False):
    ...
    if flag:
        ...
        for b in tmpjs:
            data[flag]= str(b['key'])
            getAgg(b,glist,vlist,data,getcount)
            #print data
    else:
...
def aggJson2csv(args):
    ...
    js = js['aggregations']
    data={}
    getAgg(js,glist,dlist,data,args.count)
    if args.outputfile:
        f = open(args.outputfile,'w')
        for d in aggres:
            f.write(','.join(d)+'\n')
...
def main():
    ...
    parser_b.add_argument('-c', '--count',action='store_true',  help='get doc_count')
