...
python esJson2csv -j ${tmpdir}/${topic}.json -o ${csvdir}/${topic}_${begindate}.csv -l logtime,host,ip,app,ses,addr,auid gethit -f "hit['_source']['logtime'],hit['_source']['bimap']['host'],hit['_source']['bimap']['ip'],hit['_source']['bimap']['app'],hit['_source']['auditd']['log']['ses'],hit['_source']['auditd']['log']['msg']['addr'],hit['_source']['auditd']['log']['auid']"
...
echo ${qsl} > ${tmpdir}/search_${topic}.json
curl -X GET -u ${ESUSER}:${ESPWD} ${url}/${index}/_search  -H 'Content-Type: application/json' -d @${tmpdir}/search_${topic}.json > ${tmpdir}/${topic}.json
python esJson2csv -j ${tmpdir}/${topic}.json -o ${csvdir}/${topic}_${begindate}.csv -l logouttime,host,ses gethit -f "hit['_source']['logtime'],hit['_source']['bimap']['host'],hit['_source']['auditd']['log']['ses']"
